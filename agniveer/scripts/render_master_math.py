#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render the master Mathematics TXT from _math_raw.json."""
import json, re
...
out = []
out.append("=" * 78)
out.append("AGNIVEER VAYU (AIR FORCE) — MASTER MATHEMATICS QUESTION BANK")
out.append("All Mathematics questions from ALL available papers (Group X + Group XY + Model)")
...
out.append("are reproduced as printed. A few questions whose source text/options were")
out.append("badly garbled (matrix/fraction layout, vector symbols) were reconstructed")
out.append("into clean linear notation without changing their meaning.")
out.append("")
...
for r in R_sorted:
    ...
    tot_q += cnt
    out.append(f"  {r['key']:12s} | {r['grp']:3s} | {r['label']:22s} | {r['src']:12s} | Maths Qs: {cnt}")
out.append(f"  {'':12s} | {'':3s} | {'':22s} | {'':12s} | TOTAL Maths Qs: {tot_q}")
out.append("")
out.append("MISSING PAPERS (Mathematics section not obtainable anywhere free):")
out.append("  - Group X, 18 Mar 2024 (Shift 1) — available only on Testbook (login/paid,")
out.append("    link expired); no free site carries its Mathematics questions, so nothing")
out.append("    could be harvested. (The PW official model papers contain no Mathematics")
out.append("    section, hence no Maths questions from them.)")
out.append("")
...
txt = "\n".join(out) + "\n"
path = '/home/user/agniveer/Master_Math_All_Papers.txt'
open(path, 'w', encoding='utf-8').write(txt)
print("WROTE:", path)
print("Total maths questions:", tot_q)
